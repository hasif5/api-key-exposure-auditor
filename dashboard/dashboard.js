...
import { getSiteSecurityDb, clearSiteSecurity, deleteSiteOrigin,
  siteSeveritySummary, sevRank, CATEGORY_META } from '../lib/site-audit.js';
...
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === 'local') {
    if (changes.gaks_db || changes.gaks_collection) render();
    if (changes.gaks_site_security) renderSecurity();
  }
});
...
const secSites = document.getElementById('secSites');
const secEmpty = document.getElementById('secEmpty');
const secStats = document.getElementById('secStats');
const secFilter = document.getElementById('secFilter');
const secExpandedOrigins = new Set();
...
tabBtns.forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    tabKeys.hidden = tab !== 'keys';
    tabSecurity.hidden = tab !== 'security';
    keysToolbar.hidden = tab !== 'keys';
    if (tab === 'security') renderSecurity();
  });
});

document.getElementById('secClearBtn').addEventListener('click', async () => {
  if (!window.confirm('Clear all site security scan results?')) return;
  await clearSiteSecurity();
  renderSecurity();
  toast('Cleared all site security scans');
});

if (secFilter) secFilter.addEventListener('input', renderSecurity);

function renderSecStats(sites) {
  const total = sites.length;
  let totalIssues = 0, crit = 0, high = 0, med = 0;
  sites.forEach((s) => {
    const issues = s.issues || [];
    totalIssues += issues.length;
    const sm = siteSeveritySummary(issues);
    crit += sm.critical;
    high += sm.high;
    med += sm.medium;
  });
  secStats.innerHTML = '';
  [
    { n: total, l: 'sites scanned' },
    { n: totalIssues, l: 'total issues' },
    { n: crit, l: 'critical', alert: crit > 0 },
    { n: high, l: 'high', alert: high > 0 },
    { n: med, l: 'medium' }
  ].forEach((d) => {
    const div = document.createElement('div');
    div.className = 'stat' + (d.alert ? ' alert' : '');
    div.innerHTML = '<div class="n">' + d.n + '</div><div class="l">' + d.l + '</div>';
    secStats.appendChild(div);
  });
}

function sevBadge(severity) {
  return '<span class="sec-sev ' + severity + '">' + severity.toUpperCase() + '</span>';
}

function renderSecSite(origin, site) {
  const issues = site.issues || [];
  const sm = siteSeveritySummary(issues);
  const expanded = secExpandedOrigins.has(origin);

  const wrap = document.createElement('div');
  wrap.className = 'sec-site';

  const hdr = document.createElement('div');
  hdr.className = 'sec-site-hdr';

  const left = document.createElement('div');
  left.className = 'sec-site-left';
  let sevHtml = '';
  if (sm.critical) sevHtml += '<span class="sec-sev critical">' + sm.critical + ' critical</span>';
  if (sm.high) sevHtml += '<span class="sec-sev high">' + sm.high + ' high</span>';
  if (sm.medium) sevHtml += '<span class="sec-sev medium">' + sm.medium + ' medium</span>';
  if (sm.low) sevHtml += '<span class="sec-sev low">' + sm.low + ' low</span>';
  if (sm.info) sevHtml += '<span class="sec-sev info">' + sm.info + ' info</span>';
  left.innerHTML =
    '<span class="sec-toggle">' + (expanded ? '▼' : '▶') + '</span>' +
    '<span class="sec-origin">' + esc(origin) + '</span>' +
    '<span class="sec-count">' + issues.length + ' issue' + (issues.length !== 1 ? 's' : '') + '</span>' +
    '<span class="sec-sevs">' + sevHtml + '</span>';

  const right = document.createElement('div');
  right.className = 'sec-site-right';
  const ts = document.createElement('span');
  ts.className = 'sec-time';
  ts.textContent = 'scanned ' + fmtTime(site.lastScan);
  ...
  hdr.addEventListener('click', () => {
    if (secExpandedOrigins.has(origin)) secExpandedOrigins.delete(origin);
    else secExpandedOrigins.add(origin);
    renderSecurity();
  });
  ...
  if (expanded) {
    ...
    const cats = {};
    issues.slice().sort((a, b) => sevRank(a.severity) - sevRank(b.severity))
      .forEach((issue) => {
    ...
    const catOrder = ['headers', 'cookies', 'mixed-content', 'sri', 'dom', 'exposure'];
    const sorted = catOrder.filter((c) => cats[c]).concat(
      Object.keys(cats).filter((c) => catOrder.indexOf(c) === -1));

    sorted.forEach((cat) => {
      const meta = CATEGORY_META[cat] || { icon: '•', label: cat };
      const hd = document.createElement('div');
      hd.className = 'sec-cat-hd';
      hd.innerHTML = '<span class="sec-cat-icon">' + meta.icon + '</span> ' + esc(meta.label);
      body.appendChild(hd);

      cats[cat].forEach((issue) => {
        const card = document.createElement('div');
        card.className = 'sec-issue sev-' + issue.severity;
        const top = document.createElement('div');
        top.className = 'sec-issue-top';
        top.innerHTML = sevBadge(issue.severity) +
          '<span class="sec-issue-title">' + esc(issue.title) + '</span>';
        card.appendChild(top);
        if (issue.detail) {
          const det = document.createElement('div');
          det.className = 'sec-issue-detail';
...
async function renderSecurity() {
  ...
  let sites = Object.entries(db).map(([origin, site]) => ({ origin, ...site }));

  const q = (secFilter && secFilter.value || '').toLowerCase().trim();
  if (q) sites = sites.filter((s) => s.origin.toLowerCase().includes(q));

  sites.sort((a, b) => {
    const aMax = Math.min(...(a.issues || []).map((i) => sevRank(i.severity)).concat([5]));
    const bMax = Math.min(...(b.issues || []).map((i) => sevRank(i.severity)).concat([5]));
    return aMax - bMax || a.origin.localeCompare(b.origin);
  });

  renderSecStats(sites);
  secSites.innerHTML = '';
  secEmpty.style.display = sites.length ? 'none' : 'block';
  sites.forEach((s) => secSites.appendChild(renderSecSite(s.origin, s)));
}
