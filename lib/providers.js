...
// matched (tracked in `seen`).
export function detectGeneric(text, seen, out) {
  for (const p of GENERIC_TOKEN_PATTERNS) {
    ...
    while ((m = p.re.exec(text)) !== null) {
      ...
      out.push({ key, provider: 'unknown', reason: p.label,
        snippet: p.label + ' — ' + snippetAround(text, m.index, key.length, 100), mapsContext: false });
    }
  }
...
export function detectKeys(text) {
  ...
  for (const p of PROVIDERS) {
    ...
    while ((m = p.keyRe.exec(text)) !== null) {
      ...
      if (secret) { snippet += ' · secret: ' + secret; seen[secret] = true; } // don't re-flag the paired secret
