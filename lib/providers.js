...
function decodeLayers(text) {
  const layers = [];
  // Skip the decode pass entirely on very large blobs — the cost isn't worth it
  // and a plaintext key would already be caught by the primary scan.
  ...
  // Base64 / base64url blobs (cap how many we examine and decode + per-blob size).
